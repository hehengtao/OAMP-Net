...
weight_mat = 'C:\GT\GT works\globalsip长文工作\EP_JCD'
...
S_16QAM = [1 / np.sqrt(10), -1 / np.sqrt(10), 3 / np.sqrt(10), -3 / np.sqrt(10)]
...
def Variable4(shape):
    init = tf.constant_initializer(-np.log(4))
    #    init = tf.constant_initializer(0.2)
    dampingCE = tf.get_variable('dampingCE', shape=shape, initializer=init, dtype='float64')
    return dampingCE
...
#    a1 = phi(1 / np.sqrt(10), r, rvar) / np.sqrt(10)
#    a2 = -phi(-1 / np.sqrt(10), r, rvar) / np.sqrt(10)
#    a3 = phi(3 / np.sqrt(10), r, rvar) * 3 / np.sqrt(10)
#    a4 = -phi(-3 / np.sqrt(10), r, rvar) * 3 / np.sqrt(10)
#
#    c1 = phi(1 / np.sqrt(10), r, rvar) / np.sqrt(10) / np.sqrt(10)
#    c2 = phi(-1 / np.sqrt(10), r, rvar) / np.sqrt(10) / np.sqrt(10)
#    c3 = phi(3 / np.sqrt(10), r, rvar) * 3 / np.sqrt(10) * 3 / np.sqrt(10)
#    c4 = phi(-3 / np.sqrt(10), r, rvar) * 3 / np.sqrt(10) * 3 / np.sqrt(10)
#
#    b1 = phi(1 / np.sqrt(10), r, rvar)
#    b2 = phi(-1 / np.sqrt(10), r, rvar)
#    b3 = phi(3 / np.sqrt(10), r, rvar)
#    b4 = phi(-3 / np.sqrt(10), r, rvar)
#    a = a1 + a2 + a3 + a4
#    b = b1 + b2 + b3 + b4  # important
#    b = tf.maximum(b, eps)  # important
#    xhat = tf.divide(a, b)
#    c = c1 + c2 + c3 + c4
#    var = tf.divide(c, b) - xhat * xhat
#
#    return xhat, var
...
#        print(batch_Ydata.shape)
        ...
        for bb in range(Batch):
            for ii in range(row):  # 注意range的写法
                for jj in range(col):
                    for kk in range(len(S_16QAM)):
                        #                     print(len(S_16QAM))
                        D[kk] = np.abs(X_data_hat_[bb, ii, jj] - S_16QAM[kk])
                    index = np.argmin(D)
                    s_conste[bb, ii, jj] = S_16QAM[index]
        ...
        for bb in range(Batch1):
            for ii in range(row1):
                for jj in range(col1):
                    x_bit_hat = demapping_table_16QAM[x_conste[bb, ii, jj]]  # 这是一个tuple
                    x_bit_true = demapping_table_16QAM[x_complex_[bb, ii, jj]]  # 这是一个tuple
            ...
            for ii in range(row):  # 注意range的写法
                for jj in range(col):
                    for kk in range(len(S_16QAM)):
                        D[kk] = np.abs(X_data_hat_[bb, ii, jj] - S_16QAM[kk])
                    index = np.argmin(D)
                    s_hat_test[bb, ii, jj] = S_16QAM[index]
...
with tf.Graph().as_default():
    ...
    with tf.variable_scope('gamma'):
        gamma=Variable((iter_JCD, itermax))
        
    with tf.variable_scope('gamma1'):
        gamma1=Variable1((iter_JCD, itermax))  
        
    with tf.variable_scope('corre'):
        corre=Variable2((iter_JCD, itermax)) 
        
    with tf.variable_scope('expec'):
        expec=Variable3((iter_JCD, itermax))  

    with tf.variable_scope('dampingCE'):
        
        dampingCE = Variable4((iter_JCD, itermax))
...
#            print(tf.matrix_diag(tf.squeeze(v_B2A_inv1)))
            ...
            v_B2A_inv1 = (1 - tf.sigmoid(dampingCE[kk,tt])) * v_B2A_inv1 + tf.sigmoid(dampingCE[kk,tt]) * v_B2A_inv_new1
            
            x_B2A1 = (1 - tf.sigmoid(dampingCE[kk,tt])) * x_B2A1 + tf.sigmoid(dampingCE[kk,tt]) * x_B2A_new1            
#            v_B2A_inv1 = (1-dampingCE)*v_B2A_inv1+dampingCE*v_B2A_inv_new1
##            print(v_B2A_inv1)
#            x_B2A1 = (1-dampingCE)*x_B2A1+dampingCE*x_B2A_new1          
...
#                print(s.shape)                
                ...
                r = s + gamma[kk,tt]*tf.matmul(W,z)
#        print(r)
                noise_equal = tf.squeeze(tf.reduce_sum(Cov_H_real, [1, 2], keepdims=True))
#                print(noise_equal)
...
#                print(v2)
                ...
                B=I-gamma1[kk,t]*tf.matmul(W, HH_est)
                
                tau2 = v2/2/N*tf.trace(tf.matmul(B,B,adjoint_b=True))+gamma1[kk,t]*gamma1[kk,t]*noise_equal/2/M/4/N*tf.trace(tf.matmul(W,W,adjoint_b=True))
#                print(r)
#                print(tau2)
                s = MMSE_Constellation(r,tf.tile(tf.expand_dims(tf.expand_dims(tau2, axis=-1), axis=-1),[1,2*M,1]) ) 
#                print(s)
                s=corre[kk,t]*(s- expec[kk,t]*r) 
#                print(s)
